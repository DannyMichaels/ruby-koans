...
class AboutKeywordArguments < Neo::Koan
  ...
  # irb(main):157:0> method_with_keyword_arguments
  # => [1, "two"]
  def test_keyword_arguments
    assert_equal Array, method_with_keyword_arguments.class
    assert_equal [1, "two"], method_with_keyword_arguments
    assert_equal ["one", "two"], method_with_keyword_arguments(one: 'one') #choosing index 0, replacing 1 with "one"
    assert_equal [1, 2], method_with_keyword_arguments(two: 2)  #choosing index 1, replacing "two" with 2
  end
